...
(async () => {
    ...
    const wasm = await WebAssembly.instantiateStreaming(fetch('../../lib/tpsecore.wasm'), {
        tpsecore: {
            async fetch_asset(asset_id) {
                ...
                switch (asset_id) {
                    case 0:
                        asset = 'https://tetr.io/js/tetrio.js?bypass-tetrio-plus';
                        break;
                    case 1:
                        asset = 'https://tetr.io/sfx/tetrio.opus.rsd?bypass-tetrio-plus';
                        break;
                    case 2:
                        throw new Error("unknown asset #" + asset_id);
                }
                try {
    ...
    function log(level, tpse, msg) {
        ...
        switch (level) {
            case 1:
                console.error("wasm>", {t, tpse}, msg);
                break;
            case 2:
                console.warn("wasm>", {t, tpse}, msg);
                break;
            case 3:
                console.info("wasm>", {t, tpse}, msg);
                break;
            case 4:
                console.debug("wasm>", {t, tpse}, msg);
                break;
            case 5:
                console.debug("wasm>", {t, tpse}, msg);
                break;
            default:
                console.log("wasm>", {t, tpse}, msg);
                break;
        }
    }
    ...
    async function tpsecoreImport(input, logs) {
        ...
        if (advancedMergeLogic) {
            ...
            let p = new Promise(res => tpse_import_handlers.set(tpse, {
                finish: res, log: () => {
                }
            }));
        ...
        try {
            switch (queue_import_result) {
                case 0:
                    break;
                case 1:
                    throw new Error("general error (see logs)");
                case 2:
                    throw new Error("internal error (invalid tpse handle)");
                default:
                    throw new Error("unknown error " + queue_import_result);
            }
