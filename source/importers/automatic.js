import {load as sfxLoad, test as sfxTest} from './sfx/encodeFromFiles.js';
import {load as musicLoad, test as musicTest} from './music.js';
import {load as bgLoad, test as bgTest} from './background.js';
import genericTextures from './generic-texture.js';
import {populateImage} from '../shared/filehelper.js';
import '../shared/migrate.js';
import '../shared/tpse-sanitizer.js';
...
export default async function automatic(importers, files, storage, options) {
    ...
    if (files.every(file => file.name.endsWith('.zip'))) {
        ...
        let results = await Promise.all(files.map(async file => {
            ...
            (options && options.log || (() => {
            }))("Importing files from zip " + file.name + "...");
